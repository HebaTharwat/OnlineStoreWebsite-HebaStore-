...
?>
  ...
  <?php 
	...
	if(!empty($_POST['Colors'])) {
		$colorString = sanitize($_POST['Colors']);
...
<option value="<?php ((isset($_POST['parent']) && $_POST['parent'] == '')?' selected':'') ?>"></option>
	...
	<input type="text" id="list_price" name="list_price" class="form-control"
			...
			<button class="btn btn-default form-control" onclick="jQuery('#colorsModal').modal('toggle');return false;">Quantity & Colors </button>
		</div>

<div class="form-group col-md-3">
			<label for="colors"> Colors & Qty Preview</label>
	...
	<input type="file" name="photo" id="photo" class="form-control" value="<?php echo ((isset($_POST['Image']))?$_POST['Image']:'') ?>">
</div>
...
<input type="submit" value="Add Product" class="form-control btn btn-primary col-md-3">
       ...
       <!-- Modal -->
<div class="modal fade" id="colorsModal" tabindex="-1" role="dialog" aria-labelledby ="colorsModalLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">

      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="colorsModalLabel">Color & Quantity</h4>
      ...
      <div class="container-fluid">
        ...
        <?php for($i = 1; $i <= 12; $i++): ?>
        	...
        	<div class="form-group col-md-4">
        		<label for="colors<?php echo $i; ?>">Color:</label>
        <input type="text" class="form-control" name="colors<?php echo $i; ?>" id="colors<?php echo $i; ?>"
value="<?php echo ((!empty($cArray[$i-1]))?$cArray[$i-1]:''); ?>" >
        	</div><!-- End color -->
            
        	<div class="form-group col-md-2">
        		<label for="qty<?php echo $i; ?>">Quantity:</label>
        		<input type="number" class="form-control" min=0 id="qty<?php echo $i; ?>" name="qty<?php echo $i; ?>" 
 value="<?php echo ((!empty($qArray[$i-1]))?$qArray[$i-1]:''); ?>" >
        	</div><!-- End Quantity -->
...
<div class="modal-footer">
  ...
  <button type="button" class="btn btn-primary"
   onclick="updateColors();jQuery('#colorsModal').modal('toggle');return false;">
   Save changes</button>
